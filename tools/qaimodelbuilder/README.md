...
│  desktop/     Tauri 2.x desktop shell (optional)               │
└──────────────────────────────────────────────────────────────┘
```
...
> **Authoritative docs** live under [`docs/`](docs/) — see [`docs/README.md`](docs/README.md) for the navigation index (architecture, AI coding, UI/UX, security, operations, and the refactor record).
...
>
> Companion design docs live under [`docs/30-ui-ux/`](docs/30-ui-ux/).
...
A single Pack can ship multiple precision variants of the same logical model (contract in [`docs/30-ui-ux/multi-variant-pack-contract.md`](docs/30-ui-ux/multi-variant-pack-contract.md)). Each variant declares its own `runtime` (quantization, model size, context bins, supported devices), `assets`, and `metrics`:
...
├── tests/                # pytest suite (unit / integration / contract / e2e)
