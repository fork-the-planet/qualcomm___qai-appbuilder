...
Ran `infer_resnet50.py` on the QNN_DLC float package with the repo built-in real images. Cold start ~2.1 s, warm NPU inference ~3.1 ms.
...
| `tabletop.jpg` | teapot 8.29% / coffeepot 8.07% / cup 7.91% / mortar 5.74% / ladle 4.23% |
...
## Verified Output (X2 Elite, earlier)

| Image | Top-1 (QNN = ONNX) | Confidence | Latency |
|-------|-------------------|------------|---------|
| `1.jpg` (Samoyed dog) | Samoyed | 90.96% | ~8 ms |
| `4.jpg` (butterfly/flower) | cabbage butterfly | 27.29% | ~4 ms |

Top-5 for `1.jpg`: Samoyed 90.96% / white wolf 2.12% / Eskimo dog 1.36% / Pomeranian 1.32% / wallaby 0.49%

Top-5 for `4.jpg`: cabbage butterfly 27.29% / daisy 9.77% / bee 9.18% / sulphur butterfly 5.33% / monarch 4.33%

> `test_image.jpg` in the workspace is a **synthetic** warm-tone image (R=200,G=150,B=80 + noise, seed=42) — Top-K results on it are not meaningful.
