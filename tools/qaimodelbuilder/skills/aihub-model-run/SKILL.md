---
name: AI Hub Model Run
description: Download pre-exported models from Qualcomm AI Hub and run inference on-device. Supports QNN_CONTEXT_BINARY, QNN_DLC, VOICE_AI, ONNX, TFLITE formats. All on-device (NPU/HTP) inference runs through qai_appbuilder (QNNContext), loading the QNN context binary (.bin) or .dlc. Standard onnxruntime is used only for an optional CPU baseline comparison.
tags: aihub, inference, qualcomm, on-device, qnn, tts, asr, classification
use_for: Download pre-exported models from AI Hub and run inference on Snapdragon X Elite / X2 Elite without conversion.
homepage: "https://aihub.qualcomm.com/compute/models"
pinned: true
---

# AI Hub Model Download & Inference
...
| 2 | Read this model's `NOTES.md`? | `read skills/aihub-model-run/models/<model_id>/NOTES.md`; if it exists → must read (contains download links / I/O / known pitfalls / ready-made `infer_*.py`, lets you skip most of Step 1~5); if it does not exist → you **must create it** at the end of the task |
...
```
You will execute an aihub-model-run skill task: <specific task, e.g. "download the Inception v3 pre-compiled package and run inference on a test image on the NPU, printing Top-5">.

[Step 1, mandatory] First use the read tool to fully read:
  ${APP_ROOT}\skills\aihub-model-run\SKILL.md
...
The directory name under skills/aihub-model-run/models/ may differ from the model name in this prompt
...
NEVER guess the path or rely on a single glob — always do BOTH steps:
  1. list "skills/aihub-model-run/models/" to see the exact directory names present.
...
[Forbidden] Do not use any script/path from the model-builder skill (run_pipeline.py, qnn-onnx-converter, qairt conversion tools, etc.) — this task is a pre-compiled package, no conversion.

When done, report: chipset detection result / model package path and contents / inference script path / Top-5 results.
```
...
list "skills/aihub-model-run/models/"   ← always do this first

# Step 2: read NOTES.md using the exact name from the listing
read "skills/aihub-model-run/models/<exact_dir_name>/NOTES.md"
```
...
0.5  NOTES    read skills/aihub-model-run/models/<model_id>/NOTES.md  ← MANDATORY FIRST
