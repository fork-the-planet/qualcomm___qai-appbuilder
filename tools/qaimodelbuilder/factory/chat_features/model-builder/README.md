...
QAI ModelBuilder is a toolkit for AI model compilation and deployment on Qualcomm platforms.
...
**You say:**
> "Do you have the QAI Model Builder available?"

**Alternative Test:**
> "List the tools available in the QAI Model Builder"
...
1. The AI assistant detects the skill, for example `.cline/skills/QAI Model Builder/` in your project or `~/.cline/skills/QAI Model Builder/` globally
...
The QAI Model Builder can be tested through AI-assisted workflows. The following test scenarios are available:
...
- **QNN Inference**
  - Prompt: `"I need to use QAI Model Builder to run inference my converted model real_esrgan_x4plus.dlc on current wos device"`
  - Creates Python script using `qai_appbuilder` library
  - Prompt: `"use QAI Model Builder, create qnn inference script from @onnx_inference.py using @yolov8n_a16_w8_qnn_ctx.bin model . follow the guide strictly." `
  - prompt: `"use QAI Model Builder, create qnn inference script from @onnx_inference.py using @yolov8n_a16_w8_qnn_ctx.bin context binary model "`
  - prompt: `"use QAI Model Builder, create snpe inference script from @onnx_inference.py using @yolov8n.dlc model . "`

- **Quantize Model **
  - Prompt: `"Create W8A16 QNN quantization. Use COCO128 for calibration."`

- **Operator Patching**
  - Prompt: `"Follow QAI Model Builder and patch the model."`
...
Use the following prompts to run a complete QAI ModelBuilder workflow for the current project.

- `"Create a QAI ModelBuilder project in this folder."`
  - Stay in the current source path and create the project in place.

- Adjust the project configuration in `qai_plan.md`.
...
- `"follow qai skill" + <your work prompt>`

This forces the agent to adhere to the model-builder skill workflow and reference rules.

### Tested Scenarios

- **WoS (Windows on Snapdragon)**:
  - Convert and run inference on the same device (**Snapdragon X Elite / X2 Elite**)
