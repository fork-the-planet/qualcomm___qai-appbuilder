...
把结构化 profiling trace（如 `chromeTrace.json`）喂给 LLM，用自然语言直接问"瓶颈在哪 / 哪个阶段最耗时 / 哪里卡顿"。这是对 profiling 分析手段的补充，可直接用在 `${APP_ROOT}/factory/chat_features/model-opt/profiling/` 产出的 chromeTrace.json 上。
